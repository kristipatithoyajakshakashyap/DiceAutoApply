...
async def break_delay():
    ...
    await countdown_sleep(delay, label="Taking a break to avoid detection")
...
async def countdown_sleep(seconds: float, label: str = "Resting", log_interval: float = 60.0):
    """Sleep for `seconds`, showing a live in-place console countdown.

    Writes a carriage-return-updated line straight to stdout every second
    (bypasses the logger so it doesn't collide with the rotating
    StreamHandler's newline-per-record output), while still logging a
    start line, periodic breadcrumbs every `log_interval`s, and a final
    "done" line to the normal rotating log file.
    """
    total = max(0.0, seconds)
    if total <= 0:
        return

    logger.info(f"{label}: {_format_remaining(total)} (live countdown in console)...")
    ...
    while remaining > 0:
        ...
        sys.stdout.write(f"\r{label}: {_format_remaining(remaining)} remaining...   ")
        sys.stdout.flush()
        await asyncio.sleep(tick)
        now = loop.time()
        remaining = total - (now - start)

        if now - last_logged >= log_interval and remaining > 0:
            logger.info(f"{label}: {_format_remaining(remaining)} remaining...")
            last_logged = now
...
# ── Contract Subtype Classification (W2 vs C2C) ─────────────────
_C2C_PATTERNS = [re.compile(r"corp[\s-]*to[\s-]*corp"), re.compile(r"\bc2c\b")]
_W2_PATTERNS  = [re.compile(r"\bw[\s-]?2\b")]


def classify_contract_subtype(text: str):
    """Classify employment-type text from a Dice job detail page.

    Returns 'c2c' if Corp-to-Corp language is found, 'w2' if W2 language
    is found, or None if neither (or both) are found — ambiguous cases
    are left unclassified rather than guessed.
    """
    if not text:
        return None
    t = text.lower()
    has_c2c = any(p.search(t) for p in _C2C_PATTERNS)
    has_w2  = any(p.search(t) for p in _W2_PATTERNS)
    if has_c2c and has_w2:
        return None
    if has_c2c:
        return "c2c"
    if has_w2:
...
class ApplicationTracker:
    ...
    def log(self, title, company, location, url, status, notes=""):
        ...
        logger.info(f"[{status.upper()}] {title}{suffix}")
